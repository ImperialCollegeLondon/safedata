...
get_taxonomy <- function(obj){
    ...
    #' @seealso \code{\link{add_taxa}} and 
...
add_taxa <- function (obj, taxon_field=NULL, taxon_table=NULL, prefix=NULL, which=NULL) {
	...
	#' \code{\link{get_taxonomy}} that can be linked to rows in data worksheets 
	...
	#' included in the output of \code{\link{get_hierarchy}}, but \code{which} 
	...
	#' @param record A record set giving a single record for which to generate
	#'    a taxonomy table
    ...
    if(! inherits(obj, 'safe_data')){
        stop("add_taxa requires an object of class 'safe_data'")
    }
    ...
    if(is.null(taxon_table)){
        taxon_table <- get_taxonomy(obj_attr$safe_record_set)
    } else if(! inherits(taxon_table, 'safe_taxa')){
        stop("'taxon_table' not a 'safe_taxa' object created by 'get_taxonomy'")
    }
