...
You can browse published SAFE dataset at either our [SAFE Zenodo
community](https://zenodo.org/communities/safe) or on the [SAFE Project
website](https://www.safeproject.net/datasets/view_datasets). Datasets are identified by their
Zenodo record number, which is the number included in both the dataset DOI and Zenodo URL. All of
the following point to the same dataset:
...
Once you have found datasets that you want to explore further, then you can use the
...
This object is just a data frame containing validated record ids and access information and so you
can use the normal data frame indices (e.g. `recs[1,]`) to select particular records. 
...
The `safedata` package also contains a set of search functions to explore datasets. These functions
...
```{r}
print(soil_datasets <- search_text('soil'))
show_concepts(soil_datasets[2,])
```

## Taxon search details

Published datasets contain a taxonomic index of any organisms referred to within the data - see
[here](https://safe-dataset-checker.readthedocs.io/en/latest/data_format/taxa/) for details of the
...
Some data formatting takes place based on [field types](https://safe-dataset-checker.readthedocs.io/en/latest/data_format/data/#field-types): categorical variables are converted to factors; dates and datetimes are converted to `POSIXct` and times are converted to `chron::time` objects. 
