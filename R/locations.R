load_gazetteer <- function(){
	...
	#' }
	#'
	#' When this function is first called in a session, the loaded \code{sf} 
	#' object is cached in the internal safedata environment for re-use.
	#' 
	#' @return An \code{sf} object containing the SAFE gazetteer locations.
	#' @export
...
get_locations <- function(obj, gazetteer_info=FALSE){
	...
	#'   returned \code{sf} object. See \code{\link{load_gazetteer}} for details.
	...
	#'   \code{get_locations} will return NULL.
    #' @seealso \code{\link{add_locations}}
    #' @export
...
add_locations <- function (obj, location_field=NULL, location_table=NULL, gazetteer_info=FALSE) {
	...
	#' converting it into a \code{\link{[sf]sf}} spatial features object.
	#' 
    #' @param obj An existing object of class \code{safe_data}
	#' @param location_field The name of a location field in a \code{safedata} for 
	#'    which to add location data.
	#' @param location_table An existing location table for a dataset, as generated by 
	#'    \code{get_locations}.
	#' @param gazetteer_info Should all the gazetteer fields be included in the 
	#'   returned \code{sf} object. See \code{\link{load_gazetteer}} for details.
    #' @return A modified \code{safe_data} 
    #' @export
    
    if(! inherits(obj, 'safe_data')){
        stop("add_locations requires an object of class 'safe_data'")
    }
    
    # Get the location fields and check it
    obj_attr <- attr(obj, 'safe_data')
    ...
    attr(ret, 'safe_data') <- attr(obj, 'safe_data')
    class(ret) <- c('safe_data', class(ret))
